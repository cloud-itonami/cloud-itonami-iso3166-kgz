...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (KGZ, clean) ==")
    ...
    (println "== filing/submit eng-4 (declared general-category probation period is 4 months, above Labour Code Art.24's 3-month ceiling -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up TIN/INN-unregistered) ==")
    ...
    (println "== filing/submit eng-5 (TIN/ИНН registration unverified -> HARD hold) ==")
    ...
    (println "== filing/submit eng-6 (declared senior-management probation period is 6 months, exactly WITHIN Labour Code Art.24's ceiling -- clean submit, still escalates for approval) ==")
