(ns marketentry.marketentryllm
  ...
  the Kyrgyz Republic (KGZ) public-sector market-entry compliance
  actor.
  ...
  cited), never a committed record or a real zakupki.gov.kg tender
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world zakupki.gov.kg tender
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "inn-verified?=" (:inn-verified? e)
